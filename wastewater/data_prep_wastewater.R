...
waste <- read.csv("WASTECXN_2014.csv", head = TRUE); head(waste)

waste_sum_a <- waste %>% select(iso, country, WASTECXN.2012) %>% mutate(year = 2014) # Taking out country code and index score; making into long format; 
...
# NOTE: Fixed -9999 with NAs, but there's probalby a more elegant way to do it, e.g.,: 
# 
# for (j in 1:numc){
#   sel <- which(new[,j] == "-9999")
#   new[sel,j] <- "NA"
# }
# 
# for (j in 1:numr){
#   sel <- na.remove(waste_sum)
#   new[sel,j] <- NA
